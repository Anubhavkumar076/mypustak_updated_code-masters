...
public class Wishlist extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        move_cart.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if(wishlistListAllData.isEmpty())
                {
                    Toast.makeText(getApplicationContext(),"Please Select Books",Toast.LENGTH_SHORT).show();
                }
                else
                {
                    for(int i=0;i<wishlistListAllData.size();i++)
                    {

                        progressBar.setVisibility(View.VISIBLE);
                        WishlistData wishlistData=wishlistListAllData.get(i);
                        int book_id=wishlistData.getBook_id();
                        addtocart();
                    }
    ...
    private void initialisation() {
        ...
        mRecyclerView=findViewById(R.id.wishlist_recycler);
    }
